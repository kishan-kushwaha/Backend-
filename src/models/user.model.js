...
userSchema.pre("save", async function() {  // async or await isliye use krte hai taki password ko hash krne me time lagta hai aur hum chahte hai ki jab tak password hash na ho jaye tab tak user ko response na bheje
     if(!this.isModified("password")) return;
    
     this.password = await bcrypt.hash(this.password, 10);  // is line ka matlab hai ki password ko hash krna hai aur 10 rounds of salt use krna hai taki password ko crack karna mushkil ho jaye
      // iska means hai ki ye function complete ho gaya hai aur next middleware me ja sakta hai, yaha jab bhi data sav hoga to ye function call hoga aur password ko hash krke database me save karega
...
userSchema.methods.isPasswordCorrect =  async function(password){
    return await bcrypt.compare(password, this.password); // is line ka matlab hai ki password ko compare krna hai jo user ne input kiya hai aur jo database me save hai, agar dono match karte hai to true return karega warna false return karega
}
